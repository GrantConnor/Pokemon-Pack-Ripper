...
async function tryAcquireSpawnRefreshLock(database, now) {
  const meta = database.collection('system_meta');
  const lockUntil = new Date(now + SPAWN_REFRESH_LOCK_MS).toISOString();

  try {
    const result = await meta.findOneAndUpdate(
      {
        _id: 'wildSpawnRefreshLock',
        $or: [
          { lockUntil: { $exists: false } },
          { lockUntil: { $lt: new Date(now).toISOString() } },
        ],
      },
      {
        $set: {
    ...
    return result?.value?.lockUntil === lockUntil;
  } catch (error) {
...
async function releaseSpawnRefreshLock(database) {
  await database.collection('system_meta').updateOne(
    { _id: 'wildSpawnRefreshLock' },
    { $set: { lockUntil: new Date(0).toISOString() } },
    { upsert: true }
  );
}

function shouldCreateFreshSpawn(globalSpawn, now) {
  return !globalSpawn
...
export async function updateGlobalSpawn(database) {
  const now = Date.now();
  let globalSpawn = await database.collection('global_spawn').findOne({ id: 'current' });
  let outbreak = normalizeOutbreakState(globalSpawn, now);

  if (!shouldCreateFreshSpawn(globalSpawn, now)) {
  ...
  const lockAcquired = await tryAcquireSpawnRefreshLock(database, now);
  if (!lockAcquired) {
    return (await database.collection('global_spawn').findOne({ id: 'current' })) || globalSpawn;
  }

  try {
    globalSpawn = await database.collection('global_spawn').findOne({ id: 'current' });
    outbreak = normalizeOutbreakState(globalSpawn, Date.now());

    if (!shouldCreateFreshSpawn(globalSpawn, Date.now())) {
      return globalSpawn;
    }

    const newSpawn = await createWildSpawn(Date.now(), outbreak);
    ...
    await releaseSpawnRefreshLock(database);
  }
}
