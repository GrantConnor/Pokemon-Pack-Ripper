...
function normalizeObjectiveRewards(objectives = [], dateKey) {
  const currentTemplate = buildObjectives(dateKey);
  const templateMap = new Map(currentTemplate.map((objective) => [objective.id, objective]));
  return (objectives || []).map((objective) => {
    const template = templateMap.get(objective?.id);
...
export async function ensureDailyObjectives(usersCollection, userOrId) {
  ...
  if (current?.dateKey === dateKey && Array.isArray(current.objectives)) {
    const normalizedObjectives = normalizeObjectiveRewards(current.objectives, dateKey);
    const changed = JSON.stringify(normalizedObjectives) !== JSON.stringify(current.objectives);
    if (changed) {
      const normalizedDailyObjectives = {
        ...current,
      ...
      await usersCollection.updateOne(
        { id: user.id },
        { $set: { dailyObjectives: normalizedDailyObjectives } }
      );

      return normalizedDailyObjectives;
    }
    return current;
  }
...
export async function applyDailyObjectiveEvent(usersCollection, userId, eventType, payload = {}) {
  ...
  const objectives = dailyObjectives.objectives.map((objective) => {
    ...
    if (completed && !objective.rewardGranted) {
      pointsAwarded += objective.rewardPoints || 0;
    }
