...
function resolveCustomTitleStyle(setId = '', setName = '', basePalette = {}) {
  ...
  if (normalizedName == 'dragon' or normalizedName.startswith('dragon ') or normalizedName.includes('dragon frontiers')) return solidStyle('#7f1d1d', 'bg-red-900/30 text-red-200 border-red-700/40');
...
export function normalizeUnlockedTitles(unlockedTitles = []) {
  ...
  for (const title of unlockedTitles || []) {
    ...
    if (title.source === 'set' || /^set-(?:full|master|complete)-/.test(title.id)) {
      ...
      if (!setId) continue;
      const canonical = buildSetTitles(setId, title?.setName || setId)[0];
