...
export function syncSetTitlesFromCollection(user = {}) {
  ...
  for (const card of collection) {
    ...
    if (!grouped.has(setId)) {
      grouped.set(setId, {
        ...
        printedTotal: Number(card?.set?.printedTotal || 0),
  ...
  for (const group of grouped.values()) {
    ...
    const fullSetCompleted = group.printedTotal > 0 && group.numberedIds.size >= group.printedTotal;
...
export function mergeSpecialTitlesForUsername(username = '', unlockedTitles = []) {
  ...
  if (normalized === 'grant') {
    titleMap.set('special-spheal-of-approval', {
      ...
      textClass: 'text-[#d9f1f1]',
      badgeClass: 'bg-[#d9f1f1]/20 text-[#d9f1f1] border-[#d9f1f1]/40',
    });
  }
