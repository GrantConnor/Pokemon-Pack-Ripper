...
function resolveCustomSetTitle(setId = '', setName = '') {
  ...
  if (/^dp\d+$/i.test(setId)) return 'Lord of Time and Space';
  if (/^pl\d+$/i.test(setId)) return 'Platinum';
  if (/^hgss\d+$/i.test(setId)) return 'Heart & Soul';
  if (/^col\d+$/i.test(setId)) return 'Legend Caller';
  if (/^bw\d+$/i.test(setId)) return 'Colorless';
  if (/^xy\d+$/i.test(setId)) return 'Kalos Crown Collector';
  if (/^swsh\d+$/i.test(setId)) return 'Sword and Shield';
  ...
  return String(setName || LEGACY_SET_NAMES_BY_ID[setId] || setId || 'Set Champion').trim();
}
...
function resolveCustomTitleStyle(setId = '', setName = '', basePalette = {}) {
  ...
  if (/^dp\d+$/i.test(setId)) return solidStyle('#f0f8ff', 'bg-[#f0f8ff]/15 text-[#f0f8ff] border-[#f0f8ff]/35');
  if (/^pl\d+$/i.test(setId)) return solidStyle('#4ca275', 'bg-[#4ca275]/20 text-[#dffbea] border-[#4ca275]/40');
  if (/^hgss\d+$/i.test(setId)) return gradientStyle('bg-gradient-to-r from-amber-400 to-pink-300 bg-clip-text text-transparent', 'bg-gradient-to-r from-amber-400/20 to-pink-300/20 text-white border-amber-300/40');
  if (/^col\d+$/i.test(setId)) return solidStyle('#60a5fa', 'bg-blue-500/20 text-blue-200 border-blue-400/40');
  if (/^bw\d+$/i.test(setId)) return gradientStyle('bg-gradient-to-r from-black via-gray-400 to-white bg-clip-text text-transparent', 'bg-gradient-to-r from-black via-gray-500 to-white text-white border-gray-400/50');
  if (/^xy\d+$/i.test(setId)) return gradientStyle('bg-gradient-to-r from-red-500 to-blue-500 bg-clip-text text-transparent', 'bg-gradient-to-r from-red-500/20 to-blue-500/20 text-white border-cyan-300/40');
  if (/^swsh\d+$/i.test(setId)) return gradientStyle('bg-gradient-to-r from-slate-200 to-blue-400 bg-clip-text text-transparent', 'bg-gradient-to-r from-slate-200/20 to-blue-400/20 text-white border-blue-300/40');
