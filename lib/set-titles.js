...
export function evaluateSetCompletion(userCollection = [], allCards = [], setId) {
  ...
  if (!fullSetCompleted && totalCards > 0 && uniqueIds.size >= totalCards) {
    fullSetCompleted = true;
  }
...
export function syncSetTitlesFromCollection(user = {}, setCatalog = []) {
  ...
  for (const card of collection) {
    ...
    if (!grouped.has(setId)) {
      ...
      grouped.set(setId, {
        ...
        printedTotal: Number(card?.set?.printedTotal || catalogSet.printedTotal || 0),
        total: Number(card?.set?.total || catalogSet.total || 0),
        uniqueIds: new Set(),
        numberedIds: new Set(),
      });
    }
    ...
    const parsed = parseCardNumber(card);
    if (parsed !== null && group.printedTotal && parsed <= group.printedTotal && card?.id) {
  ...
  for (const group of grouped.values()) {
    ...
    const fullSetCompleted = (group.total > 0 && group.uniqueIds.size >= group.total) || (group.printedTotal > 0 && group.numberedIds.size >= group.printedTotal);
    const masterSetCompleted = group.total > 0 && group.uniqueIds.size >= group.total;
