...
const SET_PRICING = {
  ...
  'ex13': { single: 150, bulk: 1500 }, 'ex14': { single: 150, bulk: 1500 }, 'ex15': { single: 150, bulk: 1500 }, 'ex16': { single: 150, bulk: 1500 },
};
...
export function normalizeCardRarity(card) {
  ...
  if (rawRarity === 'Rare Shiny') return 'Rare Shiny';
  if (rawRarity === 'Rare Holo V') return 'Rare Holo V';
  if (rawRarity === 'Rare Holo VMAX') return 'Rare Holo VMAX';
...
export function getCollectibleCards(cards = []) {
  return (Array.isArray(cards) ? cards : []).filter(isCollectibleCard);
}
