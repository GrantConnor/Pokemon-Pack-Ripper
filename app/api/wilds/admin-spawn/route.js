  ...
  try {
    ...
    if (pokemonId && !resolvedPokemonId) {
      return NextResponse.json(
        { error: 'Could not find a Pokémon matching that dex number or name' },
        { status: 404 }
      );
    }

    const spawnId = resolvedPokemonId || (Math.floor(Math.random() * MAX_POKEMON_ID) + 1);
    const pokemonData = await fetchPokemonData(spawnId, false);
    ...
    const newSpawn = {
      id: 'current',
      pokemon: normalizeStoredSprite({ ...pokemonData, id: spawnId }),
      spawnedAt: Date.now(),
      nextSpawnTime: null,
      caughtBy: null,
      catchAttempts: {},
    ...
    return NextResponse.json({
      ...
      message: `Spawned ${newSpawn.pokemon.displayName} (#${spawnId})!`,
    });
  } catch (error) {
    return NextResponse.json(
      { error: error?.message || 'Failed to spawn Pokémon' },
      { status: 500 }
    );
  }
}
