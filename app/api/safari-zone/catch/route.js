...
export async function POST(request) {
  try {
    ...
    if (caught) {
      ...
      for (const key of ['spawnId', 'snackApplied', 'status', 'catchRate', 'attemptsUsed', 'maxAttempts']) {
        delete caughtPokemon[key];
      }
    ...
    if (attemptsRemaining > 0) {
      ...
      return NextResponse.json({
        ...
        caught: false,
        escaped: false,
        spawn: { ...updatedSpawn, catchRate },
        attemptsRemaining,
        message: `${spawn.displayName} broke free! ${attemptsRemaining} catch ${attemptsRemaining === 1 ? 'try' : 'tries'} remaining.`,
      });
    }
