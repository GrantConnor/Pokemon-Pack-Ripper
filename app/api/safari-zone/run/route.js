...
export async function POST(request) {
  try {
    ...
    const result = await instances.updateOne(
      ...
      {
        $set: { currentSpawn: null, nextSpawnAt, updatedAt: Date.now() },
        $push: { encounterLog: { displayName: spawn.displayName, sprite: spawn.sprite, catchRate, outcome: 'ran', isShiny: !!spawn.isShiny, safariRarity: spawn.safariRarity, createdAt: Date.now() } }
      }
    );
    if (!result.modifiedCount) return NextResponse.json({ error: 'That Safari Zone Pokémon is no longer available' }, { status: 409 });
    return NextResponse.json({ success: true, nextSpawnAt, message: `You ran from ${spawn.displayName}.` });
  } catch (error) {
