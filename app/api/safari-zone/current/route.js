...
export async function GET(request) {
  try {
    ...
    const spawnDueSoon =
      ...
      instance.nextSpawnAt &&
      (instance.nextSpawnAt - now) <= 5000;
    ...
    if (shouldRespawn) {
      ...
      const configuredBiome =
        SAFARI_BIOMES.find((entry) => entry.key === instance.biomeKey) || fallbackBiome;

      const newSpawn = await createSafariSpawn(configuredBiome, {
        shinyRate: instance.shinyRate || undefined,
      });
      ...
      const spawnResult = await instances.updateOne(
        {
          userId,
          currentSpawn: null,
      ...
      if (spawnResult.modifiedCount) {
        instance = { ...instance, currentSpawn, nextSpawnAt: null };
      } else {
        instance = await instances.findOne({ userId });
      }
    }
    ...
    const spawn = instance.currentSpawn
      ? {
          ...
          maxAttempts:
            instance.currentSpawn.maxAttempts ||
            ((instance.currentSpawn.safariRarity === 'legendary' ||
              instance.currentSpawn.safariRarity === 'mythical')
              ? 3
              : 1),
        }
      : null;
